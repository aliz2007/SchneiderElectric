...
Per-cluster justifications: for every capability cluster, each evaluator leaves one written justification. Self-assessors write a concrete example evidencing their ratings (guided to cover situation, actions taken, results, impact and, where relevant, replication); managers and the APEX Panel write a free justification note. These appear as "themeNotes" (or "justification") on the data — call them clusters, never "themes", when you write back and are the qualitative backing for the scores, so quote them when a question asks "why" or for evidence, never fabricate them.

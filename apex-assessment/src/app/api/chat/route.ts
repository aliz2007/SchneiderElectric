...
How to read the data: levels are L1 Developing, L2 Proficient, L3 Advanced. Each AM is on a track (Acquisition or Saturation) and belongs to a customer segment (Power & Grid, Energy & Chemicals, CS&P - Cloud & Service Providers, or Multi-segment). The APEX Panel is the authoritative lens. A STRENGTH is a capability where the panel is STRICTLY ABOVE the required level; a capability merely AT the required level is on the baseline and is NOT a strength. A SKILL GAP is a capability where the panel is BELOW the required level. A PERCEPTION GAP is where self and panel differ by a full level or more (over-rates = self above panel; under-rates = self below panel). Only submitted assessments carry scores.

Per-theme justifications: for every capability cluster, self-assessors answer the five APEX framework questions (Situation, Actions, Results, Impact, Replication) to evidence the levels they picked, while managers and the APEX Panel leave one written justification per theme. These appear as "themeNotes" (or "justification") on the data and are the qualitative backing for the scores — quote them when a question asks "why" or for evidence, never fabricate them.
